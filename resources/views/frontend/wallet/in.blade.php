  ...
  <form action="{{ route('front.wallet_index') }}" method="post" id="wallet_index_form">
    ...
    <section class="total">
      ...
      <div class="coin">
        ...
        <div style="width: 65%">
          <!-- Auth::guard('web')->user()->total_coins+ -->
          <input type="number" readonly value="{{Auth::guard('web')->user()->total_coins+$totalEarningCoins}}"
            id="total_coin" name="total_coin" style="width: 100%" />
        </div>
        <div style="width: 25%">
          <div class="refresh_btn">Refresh</div>
        </div>
      </div>
    </section>

    <section class="total">
      <div>Transfer Coin(COIN.)</div>
      <div class="coin">
        <div style="width: 15%">COIN.</div>
        <div style="width: 65%">
          <input type="number" value="0.00" id="transfer_coin" name="transfer_coin" style="width: 100%" />
        </div>
      </div>
      <button id="wallet_in_ok" type="button"
        style="width: 100%; background-color: #68c9fa; padding: 10px; margin-top: 20px; border-radius: 5px">
        OK
      </button>
    </section>
  </form>
