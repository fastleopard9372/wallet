...
<body>
  ...
  <section class="account_default_header">
    ...
    <div class="flexgrow" style="text-align: center">Withraw Record</div>
  </section>

  <!-- Withraw Record -->
  <section class="withraw_record">
    <div class="withraw_record_list">
      @foreach($data as $item)
      <div class="withraw_record_item">
        <div class="withraw_record_time">
          <?php
            $day = date('m/d', strtotime($item->created_at));
            $time = date('H:i:s', strtotime($item->created_at));
            $kind = 'Processing';
            $class_name = 'plus_mark';
            if ($item->statue == 0) {
              $kind = 'Processing';
              $class_name = 'min_mark';
        ...
        <div class="withraw_record_content">
          <div class="border_left">
            <div class="withraw_content">
              <?php
                try {
                  $payment_method = Crypt::decryptString($item->payment_method);
              ...
              <div>COIN.<span class="{{$class_name}}"> {{abs($item->amount)}}</span></div>
            </div>
            <div class="withraw_content ft12">
              <!-- <div></div> -->
              <div class="{{$class_name}}">{{$kind}}</div>
            </div>
          </div>
        </div>
      </div>
